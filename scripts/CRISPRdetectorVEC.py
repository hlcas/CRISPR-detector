...
'''
#-------------------------------------------------
#	   File Name:	   CRISPRdetectorVEC.py
#	   Author:		   Lei Huang
#	   Date:		   2021.10.20
...
parse.add_argument("--vector",help="vector genome path in fasta format",required=True)
...
vector = os.path.abspath(args.vector)
...
if args.c1 != None:
	...
	if args.bed == None:
		os.system('sentieon driver -t '+threads+' -r '+fasta+' -i temp/'+sample_name+'.tmp.bam --algo TNscope --tumor_sample '+sample_name+param_list+' variants.vcf.gz && sync')
	else:
