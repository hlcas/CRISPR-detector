...
parse.add_argument("--amplicons_file", help="Amplicons description file. This file is a tab-delimited text file with up to 3 columns (2 required)",required=True)
...
raw_vcf['tAD'] = raw_vcf[sample_name].apply(lambda x:x.split(':')[1]+':')
raw_vcf['tALT_AD'] = raw_vcf['tAD'].apply(lambda x:int(x.split(',')[1][:-1]))

def valueDP(x,y):		
	return str(int(x.split(',')[0])+y)
   
raw_vcf['tDP'] = raw_vcf.apply(lambda row:valueDP(row['tAD'],row['tALT_AD']),axis=1)
	
def valueAF(x,y):
	return x*100/int(y)

raw_vcf['AF%'] = raw_vcf.apply(lambda row:valueAF(row['tALT_AD'],row['tDP']),axis=1)
...
with open('temp/treatment.vcf','w') as f:		
	...
	for i in range(len(raw_vcf)):
		f.write(raw_vcf['#CHROM'].values[i]+'\t'+str(raw_vcf['POS'].values[i])+'\t.\t'+raw_vcf['REF'].values[i]+'\t'+raw_vcf['ALT'].values[i]+'\t.\t.\t.\tGT:AD:DP\t'+raw_vcf['tGT'].values[i]+raw_vcf['tAD'].values[i]+raw_vcf['tDP'].values[i]+'\n')

if args.c1 != None:		
	raw_vcf['cGT'] = raw_vcf['control_'+sample_name].apply(lambda x:x.split(':')[0]+':')
	raw_vcf['cAD'] = raw_vcf['control_'+sample_name].apply(lambda x:x.split(':')[1]+':')
	raw_vcf['cALT_AD'] = raw_vcf['cAD'].apply(lambda x:int(x.split(',')[1][:-1]))
	raw_vcf['cDP'] = raw_vcf.apply(lambda row:valueDP(row['cAD'],row['cALT_AD']),axis=1)
	raw_vcf['cAF%'] = raw_vcf.apply(lambda row:valueAF(row['cALT_AD'],row['cDP']),axis=1)
	...
	with open('temp/paired.vcf','w') as g:
		...
		for i in range(len(raw_vcf)):		
			c += 1
			g.write(lines[c].strip()+'\t'+raw_vcf['cGT'].values[i]+raw_vcf['cAD'].values[i]+raw_vcf['cDP'].values[i]+'\n')
...
avinput['ALT_AD'] = raw_vcf['tALT_AD']
avinput['AF%'] = raw_vcf['AF%']

if os.path.exists('temp/paired.vcf'):
	avinput['control'] = raw_vcf['cAF%']
...
if len(avinput) != 0:
	...
	for t in avinput['amplicon'].unique():
		...
		for j in range(len(avinput_loci)):
			...
			adx = avinput_loci['ALT_AD'].values[j]
			dpx = avinput_loci['DP'].values[j]
			...
			if dlen == 0:
				for m in avinput_loci['alt'].values[j]:
					...
					pos_ad_dic[pos][m] += adx
					pos_dp_dic[pos] = max(dpx,pos_dp_dic[pos])
			elif dlen < 0:
				if dlen not in indelsize:
					indelsize[dlen] = adx
				else:
					indelsize[dlen] += adx
				...
				pos_ad_dic[pos+1]['I'] += adx
				pos_dp_dic[pos+1] = max(dpx,pos_dp_dic[pos])
		...
		with open(t+'/out_distribution_of_indel_size.csv','w') as fi:
			...
			for ki in sorted(indelsize):
				fi.write(str(ki)+','+str(indelsize[ki])+'\n')
	...
	for amp in avinput['Amplicon'].unique():
		avinput[avinput['Amplicon'] == amp].to_csv(amp+'/out_mutations.txt',sep='\t',index=None)
...
if len(amplicon_fas.keys()) > 0:
	def substitutions_removal(xloci,ydf):
		if args.ignore_substitutions == 1:
			indeldf = ydf[['POS','depth','deletion','insertion','indel','deletion%','insertion%','indel%']]		
	...
	if args.ignore_substitutions == 1:
		mapdf = mapdf.drop(['substitution','modified','substitution%','modified%'],axis=1)
	...
	#mapdf = mapdf.sort_values(by = ['indel%','n_reads'],ascending=False)
